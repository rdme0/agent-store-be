...
class QuoteService(
    ...
    fun create(slug: String, request: QuoteRequest): QuoteResponse {
        ...
        val candidates = agentVersionRepository.findAllByAgentSlugAndStatus(slug, AgentVersionStatus.ACTIVE)
        if (candidates.isEmpty()) throw ApiException("AGENT_NOT_FOUND", "Agent was not found", 404, mapOf("slug" to slug))
        ...
        val quote = quoteRepository.save(com.agentstore.dependency.model.entity.ExecutionQuote(UUID.randomUUID(), root, expiresAt, cost.maxCostAtomic, objectMapper.valueToTree(snapshot)))
