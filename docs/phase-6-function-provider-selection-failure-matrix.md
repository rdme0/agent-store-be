...
- `lowest_price`·`latest_version`·`highest_reliability`·`fastest`·`balanced` 선택은 Quote 생성 시 한 번만 수행한다.
...
|---|---|---|---|---|
| FC-01 | schema 크기·깊이·원격 `$ref`·응답 형식 불일치 | 계약/Version publish 거절 | 원격 조회, 불일치 ACTIVE Version | `AgentCapabilityServiceTest`, `AgentServiceCapabilityTest` |
...
| DEP-02 | function 후보 0개 | required quote 거절 | 임의 provider 선택 | resolver quote tests |
