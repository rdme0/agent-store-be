...
| Paid request        | call-path-derived deadline: depth 1 = 150 seconds, depth 2 = 120 seconds, depth 5 = 30 seconds; connection loss, response over 1 MiB, malformed/missing receipt | `RECONCILIATION_REQUIRED`; retain reservation and never retry automatically, including a response that continuously drips chunks | `X402PaymentConfigurationTest.depth-derived deadline`; `X402PaymentServiceTest.post signature ambiguity remains unknown`; `slow paid response cannot extend the absolute invocation deadline`                         |
...
| Startup recovery    | unknown payment or recovery error                                                               | 실행 mutation은 recovery가 끝날 때까지 닫고 unknown reservation은 해제하지 않는다                                              | existing `PaymentRecoveryStartupServiceTest` and recovery integration rows                                                                                    |
