...
invoking an Agent.
When `PAYMENT_MODE=x402`, the same Spring process holds a low-balance hot-wallet key and implements
x402 v2 `exact`
...
|---------------------|-------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Startup             | `PAYMENT_MODE=x402` with absent, malformed, zero, or out-of-range private key                   | Fail bean creation before serving requests; never echo the key                                                                   | `X402PaymentClientConfigurationTest`; `X402Eip3009SignerTest.zero and out of range private keys are rejected`                                                  |
...
| JVM restart         | process dies after signing/sending but before the catch marks the still-`REQUIRED` x402 attempt | Startup recovery promotes `X402 + REQUIRED` to `RECONCILIATION_REQUIRED`; retain reservation and never infer non-payment         | `PostgresSettlementRecoveryIntegrationTest.startup recovery promotes unmarked required x402 payment and retains reservation`; simulated-required contrast test |
