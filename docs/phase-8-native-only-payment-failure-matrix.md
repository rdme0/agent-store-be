...
| PAY-08A | One root Agent invokes independent sibling providers | EIP-3009 uses a distinct random authorization nonce per payment, so independent callbacks run in parallel; any callback failure cancels siblings and terminalizes once | `demo-agent/internal/runtime/client/callback_client_test.go` parallel callback test + HTTP execution E2E |
| PAY-08B | A paid nested Agent graph approaches its allowed depth | A node has a 30-second budget, while a callback transport uses `remaining depth × 30 seconds`: depth 2 has 120 seconds and depth 5 has 30 seconds. Spring derives the same budget from its call path; Go rejects a configured graph limit other than the fixed cross-service contract of 5. Thus one outer callback cannot cancel a valid nested subtree at the old 30-second boundary. | `X402PaymentConfigurationTest` depth-derived deadline test + `demo-agent/internal/runtime/client/callback_client_test.go` nested HTTP fixture + `demo-agent/internal/config/config_test.go` depth mismatch rejection |
